...
-- Trigger to automatically track learning activities
...
BEGIN
  ...
  INSERT INTO learning_activities (user_id, activity_date, lessons_completed, courses_accessed)
  VALUES (
    NEW.user_id,
    CURRENT_DATE,
    1,
    ARRAY[NEW.course_id::text]
  )
  ON CONFLICT (user_id, activity_date)
  DO UPDATE SET
    lessons_completed = learning_activities.lessons_completed + 1,
    courses_accessed = array_append(
      CASE 
        WHEN NEW.course_id::text = ANY(learning_activities.courses_accessed) 
        THEN learning_activities.courses_accessed
        ELSE array_append(learning_activities.courses_accessed, NEW.course_id::text)
      END,
      NULL
    ),
    updated_at = now();
...
-- Create trigger for enrollment tracking
DROP TRIGGER IF EXISTS track_learning_on_enrollment ON enrollments;
CREATE TRIGGER track_learning_on_enrollment
  AFTER INSERT ON enrollments
  FOR EACH ROW
  EXECUTE FUNCTION track_learning_activity();

-- Initialize funnel progress for existing users
INSERT INTO funnel_progress (user_id, stage)
SELECT id, COALESCE(funnel_stage, 'Idea')
FROM users
WHERE id NOT IN (SELECT user_id FROM funnel_progress)
ON CONFLICT DO NOTHING;

-- Sample achievements for new users
DO $$
DECLARE
  user_record RECORD;
BEGIN
